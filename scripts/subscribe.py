...
async def main(session: aiohttp.ClientSession | None = None) -> int:
    ...
    if session is None:
        async with aiohttp.ClientSession() as local_session:
            return await run_subscribe(settings, min_score, window_hours, dry_run, local_session)
    else:
        return await run_subscribe(settings, min_score, window_hours, dry_run, session)
...
async def run_subscribe(
    ...
    for row in candidates:
        ...
        if dry_run:
            ...
            continue
        if await github.follow(settings, row["profile"], session):
